...
| 필드 | 타입 | 설명 |
|------|------|------|
| `claudeConnected` | `boolean` | Claude OAuth 연동 여부 (`claudeAccount`가 `"oauth_connected"`이면 true) |
| `claudeSubscriptionType` | `string \| undefined` | 연동된 Claude 구독 유형 (예: `"pro"`, `"max"`, `"team"`) — 연동된 경우에만 반환 |
...
> **참고:** Claude OAuth 연동은 `POST /api/auth/claude/start` 흐름으로 별도 처리한다. `claudeAccount` 필드는 settings에서 더 이상 수정하지 않는다.
...
### 1.5 POST /api/auth/claude/start
Claude OAuth PKCE 인증 흐름을 시작한다.
PKCE code_verifier와 state는 서버 세션에만 저장되며 응답에 포함되지 않는다 (보안).
클라이언트는 반환된 `authUrl`을 새 탭/창에 열어 사용자가 인증하도록 안내한다.
...
```json
{
  "authUrl": "https://claude.ai/oauth/authorize?response_type=code&client_id=9d1c250a-...&code_challenge=...&code_challenge_method=S256&state=..."
}
```

| 필드 | 타입 | 설명 |
|------|------|------|
| `authUrl` | `string` | 사용자가 열어야 할 Claude OAuth 인증 URL. PKCE + state 포함 |

> **보안:** code_verifier와 state는 세션에만 저장. API 응답에 포함하지 않음.
...
### 1.6 POST /api/auth/claude/callback
사용자가 Claude OAuth 페이지에서 복사한 code를 수신하여 토큰을 교환하고 연동을 완료한다.
code 전체 URL 또는 code 값 단독 모두 허용 (서버가 자동 추출).
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `code` | `string` | 예 | Claude OAuth callback URL 전체 또는 code 파라미터 값. 최대 2000자 |
...
| HTTP 상태 | code | 설명 |
|-----------|------|------|
| 400 | `OAUTH_SESSION_EXPIRED` | OAuth 세션 만료 (start부터 재시작 필요) |
| 400 | `OAUTH_EXCHANGE_FAILED` | Claude OAuth 서버와의 토큰 교환 실패 |
| 401 | `UNAUTHORIZED` | 인증되지 않은 요청 |

---

### 1.7 POST /api/auth/claude/disconnect
Claude OAuth 연동을 해제한다. `CLAUDE_CONFIG_DIR` 내 `credentials.json`을 삭제하고 DB `claudeAccount`를 null로 초기화한다.
...
### 1.8 GET /api/auth/claude/status
현재 사용자의 Claude OAuth 연동 상태를 조회한다.
토큰 값 자체는 응답에 포함하지 않음 (보안).
...
| `subscriptionType` | `string \| undefined` | 구독 유형 — 연동 시에만 반환 |
| `expiresAt` | `number \| undefined` | 토큰 만료 시각 (Unix timestamp, 초 단위) |
| `isExpired` | `boolean \| undefined` | 토큰 만료 여부 |
...
> **디렉토리 매핑:** 폴더는 Git 레포지토리 내 실제 디렉토리와 매핑된다. 폴더 생성 시 프로젝트의 Git 레포에 해당 디렉토리가 자동 생성되며, 폴더 소속 세션의 worktree 작업 디렉토리는 해당 폴더 디렉토리로 스코핑된다. 폴더 미소속(프로젝트 직속) 세션은 worktree 루트에서 작업한다.
...
> **참고:** 폴더 생성 시 `dirName`은 `name`을 파일시스템 안전 문자열로 변환하여 자동 생성된다. 프로젝트 Git 레포에 해당 디렉토리가 실제로 생성된다.
...
> **참고:** `name`을 수정해도 `dirName`(실제 디렉토리명)은 변경되지 않는다. 디렉토리 매핑은 최초 생성 시 결정된다.
...
> **주의:** 폴더 삭제 시 하위 세션과 메시지가 모두 삭제된다. Git 레포 내 실제 디렉토리는 삭제하지 않는다.
...
> - Claude OAuth 미연동 사용자는 `403 CLAUDE_NOT_CONNECTED` 에러가 반환된다.

**에러 응답:**
| HTTP 상태 | code | 설명 |
|-----------|------|------|
| 400 | `BAD_REQUEST` | 워크트리 경로 미설정 |
| 401 | `UNAUTHORIZED` | 인증되지 않은 요청 |
| 403 | `CLAUDE_NOT_CONNECTED` | Claude OAuth 미연동 (`POST /api/auth/claude/start`로 연동 필요) |
| 404 | `NOT_FOUND` | 세션 없음 |
| 409 | `SESSION_LOCKED` | 다른 사용자가 세션 락 보유 중 |
| 429 | `RATE_LIMIT` | 분당 10회 초과 |
...
### CLAUDE_CONFIG_DIR 기반 인증 분리 (구독 모드)
- 구독 모드(`authMode: "subscription"`)에서는 팀원별로 별도의 `CLAUDE_CONFIG_DIR`을 설정하여 OAuth 인증 자격증명을 분리한다.
- 각 사용자의 인증 디렉토리: `{CLAUDE_CONFIGS_DIR}/{userId}/` (예: `/home/ubuntu/claude-configs/user-abc/`)
- Claude Code CLI 실행 시 해당 사용자의 `CLAUDE_CONFIG_DIR` 환경변수를 주입한다:
  ```typescript
  // 구독 모드: 사용자별 CLAUDE_CONFIG_DIR로 OAuth 인증 분리
  spawn('claude', args, {
    cwd: worktreePath,
    env: {
      ...process.env,
      CLAUDE_CONFIG_DIR: `/home/ubuntu/claude-configs/${session.creator.id}`,
    },
  });
  ```
- `api` 모드는 현재 미지원이며 차단한다. 회사 공용 API 키 사용은 보안 정책상 허용하지 않는다.

### JSONL 세션 파일 접근 (세션 이어받기)
- Claude Code JSONL 파일은 `CLAUDE_CONFIG_DIR` 내 `projects/` 디렉토리에 저장된다.
  - 경로 예: `/home/ubuntu/claude-configs/{userId}/projects/{hash}/`
- 세션 이어받기(`--resume`) 시 원래 생성자의 `CLAUDE_CONFIG_DIR`을 그대로 사용하여 JSONL에 접근한다:
  ```typescript
  // 이어받기: 생성자의 claudeSessionId + configDir 참조
  spawn('claude', ['-p', prompt, '--resume', session.claudeSessionId, ...], {
    cwd: worktreePath,
    env: {
      ...process.env,
      CLAUDE_CONFIG_DIR: `/home/ubuntu/claude-configs/${session.creator.id}`,
  ...
  ```
- 동일 `CLAUDE_CONFIG_DIR`을 공유하므로 별도의 파일 권한 설정 없이 세션 이어받기가 가능하다.

### Claude OAuth 보안 설계
- **PKCE (S256)**: code_verifier + code_challenge를 생성하여 인가 코드 탈취 공격 방어
- **state 파라미터**: CSRF 방어용 무작위 토큰. 서버 세션에 저장 후 callback에서 일치 여부 검증
- **credentials.json 파일 권한**: `chmod 600` — 소유자 읽기/쓰기만 허용
- **claude-configs 디렉토리 권한**: `chmod 700` — 소유자만 접근 가능
- **토큰 응답 제한**: access_token, refresh_token은 API 응답에 절대 노출하지 않음. 서버 파일시스템에만 저장
- **refresh_token 자동 갱신**: Claude Code CLI가 내부적으로 refresh_token을 사용하여 access_token을 자동 갱신함
