...
### 완료 기준
- [ ] Socket.IO 연결 성공 (쿠키 인증)
- [ ] 프로젝트 룸 join → 해당 프로젝트 이벤트 수신
- [ ] 이벤트 발생 → Zustand 스토어 즉시 업데이트
- [ ] 연결 끊김 → 자동 재연결
...
### 완료 기준
- [ ] 세션 진입 시 락 자동 획득 (채팅 시)
- [ ] 타인 락 세션에서 입력 불가 + 락 요청 가능
- [ ] 15분 무입력 시 락 자동 해제
- [ ] 락 상태 변경이 실시간으로 모든 클라이언트에 반영
- [ ] 락 이전(transfer) API 동작
...
### 완료 기준
- [ ] 팀원 A가 시작한 세션에서 팀원 B가 이어서 작업 가능
- [ ] `--resume`으로 Claude Code 컨텍스트 유지
- [ ] 구독 모드에서 리눅스 사용자 전환 동작
- [ ] API 모드에서 ANTHROPIC_API_KEY로 동작
